...
if(isset($_SESSION['username'])){
				...
				$id=$row[0];
				$query=" INSERT INTO ticket (customer_id_fk, no_seats, seats,payment,all_together_id,employee)VALUES ('no','$count','$t','$fare','$id','$un'); ";
				$result=mysqli_query($db,$query);
				$query=" INSERT INTO off_customer (name, number, e_id_fk)VALUES ('$name','$number','$un'); ";
				$result=mysqli_query($db,$query);
				$query=" SELECT ticket.ticket_no,ticket.no_seats,ticket.seats,ticket.payment,all_together.date,all_together.time,all_together.coach,all_together.dest,all_together.registration_fk from ticket INNER JOIN all_together on all_together.id=ticket.all_together_id where customer_id_fk='no' and no_seats='$count' and seats='$t' and payment='$fare' and all_together_id=$id and ticket.employee='$un';";
...
?>
<span>Customer Name: </span><?php echo $name ;?><br><br>
<span>Customer Number: </span><?php echo $number;?><br><br>
<span>Employee Username: </span><?php echo $un ;?><br><br>
<span>Ticket No: </span><?php echo $row2[0] ;?><br><br>
<span>Number of Seats: </span><?php echo $row2[1] ;?><br><br>
<span>Seats: </span><?php echo $row2[2] ;?><br><br>
<span>Payment: </span><?php echo $row2[3] ;?><br><br>
<span>Date of Travel: </span><?php echo $row2[4] ;?><br><br>
<span>Departure Time: </span><?php echo $row2[5] ;?><br><br>
<span>Coach Type: </span><?php echo $row2[6] ;?><br><br>
<span>Destination: </span><?php echo $row2[7] ;?><br><br>
<span>Bus Registration No.: </span><?php echo $row2[8] ;?>
<?php 
	$query=" SELECT ticket.ticket_no from ticket INNER JOIN all_together on all_together.id=ticket.all_together_id where customer_id_fk='no' and no_seats='$count' and seats='$t' and payment='$fare' and all_together_id=$id and employee='$un';";
	$res=mysqli_query($db,$query);
	$row2=mysqli_fetch_row($res);
	$un=$row2[0];
